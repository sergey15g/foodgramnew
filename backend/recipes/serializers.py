...
#                 .user.recipes_shopping_cart.all()
#                 .filter(recipe=obj)
#             )
#             != []
#         ):
...
class RecipeWriteSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        # Создаем новые ингредиенты
        for ingredient_data in ingredients_data:
            RecipeIngredientManager.create_recipe_ingredient(instance, ingredient_data)
...
class RecipeReadSerializer(serializers.ModelSerializer):
    ...
    def check_is_in_favourited(self, obj):
        ...
        if (
            self.context.get("request").user.is_authenticated
            and list(
                self.context.get("request").user.favorites.all().filter(recipe=obj)
            )
            != []
        ):
            return True
    ...
    def create(self, validated_data):
        ...
        for ingridient in ingredients_data:
            if ingridient["amount"] < 1:
                raise serializers.ValidationError(
                    {"ingredients": "Количество ингридиентов не может быть меньше 1"}
                )
...
class SubscribeSerializer(serializers.ModelSerializer):
    ...
    def get_is_subscribed(self, obj):
        ...
        if request and request.user.is_authenticated:
            return request.user.recipes_subscriptions.filter(author=obj).exists()
        return False
