...
class UserSerializer(serializers.ModelSerializer):
    ...
    def get_is_subscribed(self, obj):
        ...
        if request and hasattr(request, 'user'):
            return request.user.is_authenticated and Subscription.objects.filter(user=request.user, subscribed_to=obj).exists()
        return False
...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        extra_kwargs = {
            'avatar': {'required': False}  # Аватар не обязателен при регистрации
        }
